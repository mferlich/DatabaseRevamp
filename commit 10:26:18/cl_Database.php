...
function list_fields($table, $db_name=DB_NAME, $type=DB_TYPE) {
   return mysqli_query($this->link_identifier, 'SHOW COLUMNS FROM '.$table);
}

function field_name($field_list, $index, $type=DB_TYPE) {
  $fieldinfo=mysqli_fetch_field_direct($field_list, $index);
  //return mysql_field_name($field_list, $index);
  return $fieldinfo->name;
}

function field_type($field_list, $index, $type=DB_TYPE) {
  return mysqli_fetch_field_direct($field_list, $index);
}

function field_len($resource, $offset)
{
  return mysqli_fetch_field_direct($resource, $offset);
}
...
function num_fields($type=DB_TYPE) {
  return mysqli_field_count($this->link_identifier);
}
