...
function DB($server=DB_SERVER, $username=DB_USERNAME, $pass=DB_PASS, $name=DB_NAME, $type=DB_TYPE) {
   if(DEBUG) {
       ...
       $this->link_identifier = mysql_pconnect($server, $username, $pass);
       if ($this->link_identifier) {mysql_select_db($name);}
   }
   mysql_set_charset('latin1',$this->link_identifier);
  ...
  if (DEBUG){
      if(!$result = mysql_query($query)){
         $_SESSION['application_error'] .= "<p><strong>DATABASE ERROR</strong> (method: DB->query):"
                                        ."\n - Error number " . mysql_errno() . " . :  " . mysql_error()
                                        . "\n - Failed query: " . $query;
      }
  } else {
